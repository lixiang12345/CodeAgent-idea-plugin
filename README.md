...
For a hosted deployment, build `backend/Dockerfile` and configure either the unified model gateway contract (`MODEL_BASE_URL`, `MODEL_API_KEY`) or a fixed multi-provider allowlist using the `OPENAI_*`, `GROK_*`, and `ANTHROPIC_*` variables documented in `backend/.env.example`. Also provide `DATABASE_URL`, `OIDC_ISSUER`, `OIDC_CLIENT_ID`, `OIDC_CLIENT_SECRET`, `PUBLIC_BASE_URL`, and `SESSION_SIGNING_KEY`. The optional cloud surfaces are configured separately: `SHARE_BASE_URL` and `SHARE_LINK_TTL_SECONDS` enable shareable read-only session links, `NOTIFICATIONS_JSON` publishes operator notifications, and `SUBSCRIPTION_PLANS_JSON` with `SUBSCRIPTION_PLAN_CLAIM`, `SUBSCRIPTION_DEFAULT_PLAN`, and `SUBSCRIPTION_WARN_RATIO` reports plan and quota state. While a group is unset, sharing answers `503` with its reason, notifications return an empty list, and subscription state is `unknown` with a reason. The service exposes public OIDC bootstrap and read-only shared-session endpoints plus authenticated account, subscription, notification, model, tool, conversation, sharing, job, completion, and Agent APIs.
