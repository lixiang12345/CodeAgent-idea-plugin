...
test("does not require local project verification after approved GitHub remote-state mutations", async () => {
  const cases = [
    ["github_manage", {
  ...
  for (const [toolName, args] of cases) {
    const turns = [
      { content: "Applying approved GitHub operation", toolCalls: [{ id: "github-1", name: toolName, arguments: JSON.stringify(args) }] },
      { content: "GitHub operation completed", toolCalls: [] },
    ];
    const events = [];
    const runner = new AgentRunner({ modelGateway: { async stream() { return turns.shift(); } } });
    ...
    await runner.run({
      request: {
        mode: "agent",
        messages: [{ role: "user", content: "Apply the approved GitHub operation" }],
        tools: [{ name: toolName, description: "Write GitHub state", parameters: {}, risk: "mutating" }],
        workspace: {},
      },
      agentProfile: { ...builtInAgentProfile("loop"), maxTurns: 4 },
      emit: (type, data) => events.push({ type, data }),
      awaitToolResult: async () => ({ status: "completed", output: "completed", summary: "Completed" }),
      signal: new AbortController().signal,
    });

    assert.equal(events.some((event) => event.type === "verification.updated"), false, toolName);
    assert.equal(events.at(-1).type, "run.completed");
  }
});
...
test("serves the public OpenAPI contract", async () => {
  ...
  try {
    ...
    assert.ok(contract.paths["/v1/conversations"].get);
    assert.ok(contract.paths["/v1/conversations"].post);
    assert.ok(contract.paths["/v1/conversations/{id}"].get);
    assert.ok(contract.paths["/v1/conversations/{id}"].put);
    assert.ok(contract.paths["/v1/conversations/{id}"].delete);
    ...
    assert.equal(contract.paths["/v1/conversations"].get.responses["200"].content["application/json"].schema.$ref, "#/components/schemas/ConversationList");
    assert.equal(contract.paths["/v1/conversations/{id}"].put.responses["409"].$ref, "#/components/responses/Conflict");
    assert.equal(contract.paths["/v1/conversations/{id}"].put.parameters.some((parameter) => parameter.name === "If-Match"), true);
    assert.equal(contract.components.schemas.ConversationMessage.required.includes("createdAt"), true);
    assert.equal(contract.components.schemas.ConversationTool.required.includes("updatedAt"), true);
