...
| Rules editor | Implemented | Repository Markdown, refresh, confirmed workspace-rule deletion with metadata cleanup, persisted description and trigger metadata, client-side filename/content validation, unsaved-change protection, save, and manual per-thread selection work |
